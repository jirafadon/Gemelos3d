...
import { TALLER_PLAN_REQUEST, TALLER_PLAN_RESULT, createTallerPlanRequest } from './core/taller-fabrication-events.js';
...
function installFabricationUI(){
  ...
  window.addEventListener(TALLER_PLAN_RESULT,event=>{const summary=event.detail?.plan?.summary;if(!summary){fabPlanStatus.textContent='El Taller devolvió un resultado sin resumen.';return;}fabPlanStatus.textContent=`Plan recibido: ${summary.placed??0} piezas colocadas, ${summary.rejected??0} rechazadas, ${summary.beds??0} camas.`;});
