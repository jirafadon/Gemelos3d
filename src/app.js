...
function centerGroups(groups,b){const gap=40;groups.forEach((group,index)=>{if(!group.items.length)return;const box=new THREE.Box3();group.items.forEach(item=>box.expandByObject(item.o));const ox=(groups.length-1)*(b.x+gap)/2,bedCenterX=index*(b.x+gap)-ox,dx=bedCenterX-(box.min.x+box.max.x)/2,dy=-(box.min.y+box.max.y)/2;group.items.forEach(item=>{item.o.position.x+=dx;item.o.position.y+=dy;item.o.updateMatrixWorld(true)})})}
