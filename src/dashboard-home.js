...
section.innerHTML = `
  <div class="section-title">
    ...
    <span>${projects.length ? `${projects.length} guardado${projects.length === 1 ? '' : 's'}` : 'Tu espacio de trabajo'}</span>
  </div>
  <div class="home-project-grid"></div>
`;

const grid = section.querySelector('.home-project-grid');
if (!projects.length) {
  grid.innerHTML = `
    <a class="home-empty" href="./crear-ia.html">
      <span class="home-empty-icon">＋</span>
      <div><strong>Creá tu primer proyecto</strong><small>Empezá con IA, importá un modelo o abrí el taller.</small></div>
      <b>→</b>
    </a>`;
} else {
  projects.slice(0, 3).forEach(project => {
    ...
    const date = updated && !Number.isNaN(updated.getTime())
      ? updated.toLocaleDateString('es-AR', { day: '2-digit', month: 'short' })
      : 'Guardado';
