...
function pack(list,b){const out=[];let last=null,x=0,y=0,row=0;for(const o of list){o.updateMatrixWorld(true);const q=new THREE.Box3().setFromObject(o),w=q.max.x-q.min.x,h=q.max.y-q.min.y;if(o.userData.split){last={items:[]};out.push(last);last.items.push({o,invalid:false});continue}if(w>b.x+.01||h>b.y+.01){last={items:[]};out.push(last);last.items.push({o,invalid:true});continue}if(!last){last={items:[]};out.push(last)}if(x&&x+w>b.x){x=0;y+=row+2;row=0}if(y+h>b.y){last={items:[]};out.push(last);x=0;y=0;row=0}last.items.push({o,invalid:false});x+=w+2;row=Math.max(row,h)}return out}
...
function lists(){$('pieceList').innerHTML=objects.map((o,i)=>`<div class="piece"><b>Pieza ${i+1}</b><span>${o.userData.char||'fragmento'}${o.userData.sourceIndex!=null?` · original ${o.userData.sourceIndex+1}`:''}</span></div>`).join('');$('bedList').innerHTML=beds.map((g,i)=>`<div class="bedCard"><b>🛏️ Cama ${i+1}</b><br>${g.items.filter(x=>!x.invalid).length} piezas${g.items.some(x=>x.o.userData.split)?' · incluye divisiones':''}<button class="exportBed" data-bed="${i}">Exportar STL de esta cama</button></div>`).join('');document.querySelectorAll('.exportBed').forEach(b=>b.onclick=()=>exportBed(+b.dataset.bed))}
...
async function splitMeshToBed(mesh,b,material){
  ...
  const worldBox=new THREE.Box3().setFromObject(mesh),w=worldBox.max.x-worldBox.min.x,h=worldBox.max.y-worldBox.min.y;
  ...
  const nx=Math.max(1,Math.ceil(w/b.x)),ny=Math.max(1,Math.ceil(h/b.y));
  const pieces=[];
  for(let ix=0;ix<nx;ix++)for(let iy=0;iy<ny;iy++){
    const localX0=worldBox.min.x+ix*b.x-mesh.position.x;
    const localX1=Math.min(worldBox.max.x,worldBox.min.x+(ix+1)*b.x)-mesh.position.x;
    const localY0=worldBox.min.y+iy*b.y-mesh.position.y;
    const localY1=Math.min(worldBox.max.y,worldBox.min.y+(iy+1)*b.y)-mesh.position.y;
    const bw=Math.max(.1,localX1-localX0),bh=Math.max(.1,localY1-localY0);
    const cutter=new Brush(new THREE.BoxGeometry(bw+.2,bh+.2,Math.max(.2,worldBox.max.z-worldBox.min.z+.2)));
    cutter.position.set((localX0+localX1)/2,(localY0+localY1)/2,(worldBox.min.z+worldBox.max.z)/2-mesh.position.z);
    cutter.updateMatrixWorld(true);
    try{
      const result=evaluator.evaluate(sourceBrush,cutter,INTERSECTION);
      if(!result?.geometry||!result.geometry.attributes.position||result.geometry.attributes.position.count<3)continue;
      result.geometry.clearGroups();result.geometry.computeBoundingBox();
      const rb=result.geometry.boundingBox, rw=rb.max.x-rb.min.x,rh=rb.max.y-rb.min.y;
      if(rw>b.x+.25||rh>b.y+.25)continue;
      ...
      const part=new THREE.Mesh(result.geometry,material.clone());
      part.position.copy(mesh.position);part.rotation.copy(mesh.rotation);part.scale.copy(mesh.scale);
      part.userData={...mesh.userData,split:true,splitCell:`${ix+1}/${iy+1}`,sourceIndex:mesh.userData.sourceIndex};
      pieces.push(part);
...
async function splitOversized(chars,b){const out=[];let splitCount=0;for(let i=0;i<chars.length;i++){const o=chars[i];o.updateMatrixWorld(true);const q=new THREE.Box3().setFromObject(o),w=q.max.x-q.min.x,h=q.max.y-q.min.y;if(w<=b.x+.01&&h<=b.y+.01){o.userData.sourceIndex=i;out.push(o);continue}status(`Dividiendo pieza ${i+1} en partes fabricables…`,'warn');try{const parts=await splitMeshToBed(o,b,o.material);o.geometry.dispose();o.material.dispose();parts.forEach(p=>{p.userData.sourceIndex=i;out.push(p)});splitCount+=parts.length-1}catch(e){console.error(e);o.userData.sourceIndex=i;o.userData.splitFailed=true;out.push(o)}}return{items:out,splitCount}}
async function build(){if(!(await ensureFont()))return;const text=($('text').value||'').trim(),h=Math.max(1,+$('height').value||60),d=Math.max(.8,+$('depth').value||12),gap=Math.max(0,+$('spacing').value||4),target=Math.max(0,+$('widthScale').value||0),b=bed();clearModel();bedInfo();drawBeds(b,1);cameraFit(dims(),1);if(!text){$('guideWidth').textContent='—';$('guideHeight').textContent=h.toFixed(1)+' mm';$('guideDepth').textContent=d.toFixed(1)+' mm';status('Escribí el texto arriba y tocá “Crear modelo”.','warn');return}let cursor=0,natural=0,maxH=0;const chars=[];for(const ch of [...text]){if(ch===' '){cursor+=h*.45+gap;continue}const g=geo(ch,h,d);g.computeBoundingBox();const q=g.boundingBox,w=Math.max(.01,q.max.x-q.min.x),o=new THREE.Mesh(g,new THREE.MeshStandardMaterial({roughness:.65,metalness:.05}));o.position.x=cursor;o.userData={char:ch,width:w};chars.push(o);cursor+=w+gap;maxH=Math.max(maxH,q.max.y-q.min.y)}natural=Math.max(.01,cursor-gap);const sx=target?target/natural:1,total=target||natural;chars.forEach(o=>{o.position.x*=sx;o.scale.x=sx;o.userData.width*=sx;o.updateMatrixWorld(true)});const split=await splitOversized(chars,b);objects=split.items;objects.forEach(o=>model.add(o));beds=pack(objects,b);center(beds,b);drawBeds(b,beds.length);cameraFit(dims(),beds.length);lists();$('sx').textContent=total.toFixed(1);$('sy').textContent=maxH.toFixed(1);$('sz').textContent=d.toFixed(1);$('guideWidth').textContent=total.toFixed(1)+' mm';$('guideHeight').textContent=maxH.toFixed(1)+' mm';$('guideDepth').textContent=d.toFixed(1)+' mm';$('pieces').textContent=objects.length;const failed=objects.some(o=>o.userData.splitFailed),invalid=beds.flatMap(g=>g.items).some(x=>x.invalid);if(failed||invalid)status('✕ No se pudo dividir la pieza físicamente.','bad');else if(split.splitCount)status(`✓ División física aplicada: ${split.splitCount} cortes adicionales · orden original · piezas centradas por cama.`,'ok');else status(`✓ “${text}” · orden original · centrado en cama · ${beds.length} cama${beds.length>1?'s':''}.`,'ok')}
